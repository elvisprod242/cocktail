...
import { LayoutDashboard, Beer, ClipboardList, Package, Shapes, Settings, Armchair, Users, History, FileBarChart } from 'lucide-react';
...
export const Navbar: React.FC = () => {
  ...
  const navItems = [
    { path: '/', label: 'Tableau de bord', icon: <LayoutDashboard size={24} /> },
    { path: '/pos', label: 'Caisse', icon: <Beer size={24} /> },
    { path: '/kitchen', label: 'Commandes', icon: <ClipboardList size={24} /> },
    { path: '/history', label: 'Historique', icon: <History size={24} /> },
    { path: '/reports', label: 'Rapports', icon: <FileBarChart size={24} /> },
  ...
  return (
    <>
      ...
      <nav className="hidden md:flex flex-col w-20 lg:w-64 bg-slate-900 border-r border-slate-800 h-screen fixed left-0 top-0 z-50">
        ...
        <div className="flex-1 py-4 flex flex-col gap-1 px-3 overflow-y-auto no-scrollbar">
          {navItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
