    ...
    <div className="p-4 md:p-8 h-full overflow-y-auto bg-slate-950">
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-white flex items-center gap-3">
            <Shapes className="text-bar-accent" />
            Gestion des Catégories
          </h1>
          <p className="text-slate-400 mt-1">
            Organisez votre carte avec des catégories personnalisées
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 pb-24 md:pb-0">
        {categories.map((cat) => (
          <div
            key={cat.id}
            className="bg-slate-900 border border-slate-800 p-6 rounded-xl flex flex-col items-center justify-center gap-4 relative group hover:border-bar-accent/50 transition-colors"
          >
            <div className="w-16 h-16 bg-slate-800 rounded-full flex items-center justify-center text-bar-accent shadow-inner">
              <IconComponent name={cat.icon} size={32} />
            </div>
            <h3 className="font-bold text-white text-lg">{cat.name}</h3>
            <button
              onClick={() => setDeleteConfirmId(cat.id)}
              className="absolute top-2 right-2 p-2 text-slate-600 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-colors opacity-0 group-hover:opacity-100"
            >
              <Trash2 size={16} />
            </button>
          </div>
                ...
                <div className="grid grid-cols-5 gap-3 max-h-48 overflow-y-auto p-2 bg-slate-950 rounded-xl border border-slate-800 custom-scrollbar">
                  {Object.keys(ICON_MAP).map((iconName) => (
                    <div
                      ...
                      className={`aspect-square rounded-lg flex items-center justify-center cursor-pointer transition-all ${selectedIcon === iconName ? "bg-bar-accent text-white shadow-lg shadow-bar-accent/50 scale-110" : "text-slate-500 hover:bg-slate-800 hover:text-slate-300"}`}
                    >
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div className="bg-slate-900 border border-red-500/30 w-full max-w-sm rounded-2xl shadow-2xl p-6 text-center animate-in zoom-in duration-300">
            ...
            <p className="text-slate-400 mb-6 text-sm">
              Les produits associés resteront mais n'auront plus de catégorie
              valide.
            </p>
