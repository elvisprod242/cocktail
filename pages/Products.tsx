...
const StockProgressBar: React.FC<{
  ...
  const handleStockBlur = () => {
    ...
    if (!isNaN(val) && val >= 0 && onUpdateStock && val !== stock) {
      setIsSavingStock(true);
      onUpdateStock(val);
      setTimeout(() => setIsSavingStock(false), 500);
    } else {
      setLocalStock(stock.toString());
    }
  };
  ...
  const handleThresholdBlur = () => {
    ...
    if (!isNaN(val) && val >= 0 && onUpdateThreshold && val !== threshold) {
      setIsSavingThreshold(true);
      onUpdateThreshold(val);
      setTimeout(() => setIsSavingThreshold(false), 500);
    } else {
      setLocalThreshold(threshold.toString());
    }
  };
  ...
  return (
    <div className="w-full space-y-2">
      <div className="flex justify-between items-center text-[10px] uppercase font-bold tracking-wider">
        <span className="text-slate-500">Stock / Alerte</span>
          ...
          {onUpdateStock ? (
            <div className="relative group flex items-center">
              <input
                ...
                className={`w-14 bg-slate-900/50 border ${isSavingStock ? "border-green-500 bg-green-500/10" : "border-slate-700/50 hover:border-slate-600"} rounded px-1.5 py-0.5 text-center outline-none focus:border-bar-accent focus:bg-slate-950 transition-all ${stock <= threshold ? "text-orange-500" : "text-white"}`}
                title="Modifier le stock"
              />
              <Edit2
                size={10}
                className="absolute right-1 text-slate-500 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity"
              />
            </div>
          ) : (
            <span
              className={stock <= threshold ? "text-orange-500" : "text-white"}
            >
              {stock}
            </span>
          )}
          ...
          {onUpdateThreshold ? (
            <div className="relative group flex items-center">
              <input
                ...
                onKeyDown={handleKeyDown}
                className={`w-12 bg-slate-900/50 border ${isSavingThreshold ? "border-green-500 bg-green-500/10" : "border-slate-700/50 hover:border-slate-600"} rounded px-1.5 py-0.5 text-slate-400 text-center outline-none focus:border-orange-500 focus:bg-slate-950 transition-all`}
...
export const Products: React.FC<ProductsPageProps> = ({
  ...
  const [globalStockHistory, setGlobalStockHistory] = useState<
    (StockEntry & { productName?: string })[]
  >([]);
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const catToUse =
      category || (categories.length > 0 ? categories[0].name : "Divers");
    const imageToUse =
  ...
  const handleRestockFinalConfirm = () => {
    ...
    replenishStock(
      restockProduct.id,
      qty,
      restockNote || "Réapprovisionnement manuel",
      currentUser?.id,
      currentUser?.name,
    );
    refreshData();
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 shrink-0">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Gérez vos niveaux de stock et visualisez les ruptures
          </p>
        </div>
        <div className="flex gap-3 w-full md:w-auto">
          <button
            onClick={handleOpenGlobalHistory}
            className="bg-slate-800 hover:bg-slate-700 text-white font-bold py-3 px-6 rounded-xl flex items-center gap-2 transition-colors border border-slate-700 w-full md:w-auto justify-center"
          >
            <ClipboardList size={20} className="text-blue-500" /> Journal
          </button>
          <button
            onClick={() => handleOpenModal()}
            className="bg-bar-accent hover:bg-pink-600 text-white font-bold py-3 px-6 rounded-xl flex items-center gap-2 transition-colors shadow-lg shadow-bar-accent/25 w-full md:w-auto justify-center"
          >
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 mb-6 flex flex-col xl:flex-row gap-3 items-center sticky top-0 z-10 shadow-md">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded-lg py-2.5 pl-10 pr-4 text-white focus:ring-2 focus:ring-bar-accent outline-none"
            value={searchQuery}
        ...
        <div className="flex flex-wrap items-center gap-2 w-full xl:w-auto">
          <div className="flex-1 xl:flex-none xl:w-48">
            <select
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-lg py-2.5 px-4 text-white outline-none focus:border-bar-accent cursor-pointer text-sm"
            >
          ...
          <button
            ...
            className={`relative flex items-center gap-2 px-4 py-2.5 rounded-lg font-bold text-sm transition-all border ${showLowStockOnly ? "bg-orange-500 border-orange-400 text-white shadow-lg shadow-orange-500/20" : "bg-slate-950 border-slate-800 text-slate-400 hover:text-white"}`}
          >
            <AlertTriangle
              size={16}
              className={showLowStockOnly ? "text-white" : "text-orange-500"}
            />
            <span className="hidden sm:inline">Alertes Stock</span>
            {lowStockCount > 0 && (
              <span
                className={`px-1.5 py-0.5 rounded-full text-[10px] ${showLowStockOnly ? "bg-white text-orange-600" : "bg-orange-500 text-white"}`}
              >
                {lowStockCount}
              </span>
            )}
          ...
          <div className="flex bg-slate-950 rounded-lg p-1 border border-slate-800">
            <button
              ...
              className={`p-2 rounded-md transition-colors ${viewMode === "grid" ? "bg-slate-800 text-bar-accent" : "text-slate-400 hover:text-white"}`}
            >
              <LayoutGrid size={18} />
            </button>
            <button
              onClick={() => setViewMode("list")}
              className={`p-2 rounded-md transition-colors ${viewMode === "list" ? "bg-slate-800 text-bar-accent" : "text-slate-400 hover:text-white"}`}
            >
              <List size={18} />
            </button>
          </div>
          ...
          <div className="h-64 flex flex-col items-center justify-center text-slate-500 border-2 border-dashed border-slate-800 rounded-2xl">
            ...
            {showLowStockOnly && (
              <button
                ...
                className="mt-2 text-bar-accent hover:underline font-bold"
              >
                Effacer le filtre d'alertes
              </button>
            )}
          </div>
        ) : viewMode === "grid" ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredProducts.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className={`bg-slate-900 border rounded-xl overflow-hidden shadow-lg group flex flex-col h-full transition-all ${product.isAvailable === false ? "opacity-70 grayscale-[0.3]" : ""} ${status.critical ? "border-red-500/40 ring-1 ring-red-500/20 shadow-red-500/5" : "border-slate-800 hover:border-slate-700"}`}
                >
                  <div className="relative h-32 w-full overflow-hidden bg-slate-800/50 flex items-center justify-center">
                    ...
                    <div className="absolute top-2 right-2">
                      <span
                        className={`text-[10px] font-bold px-2 py-1 rounded shadow-sm border flex items-center gap-1 backdrop-blur-md ${status.color} ${status.alert ? "animate-pulse" : ""}`}
                      >
                  ...
                  <div className="p-4 flex-1 flex flex-col">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <span className="font-bold text-bar-accent bg-slate-950 px-2 py-1 rounded border border-slate-800 text-xs">
                    ...
                    <div className="mb-6">
                      <StockProgressBar
                        ...
                        onUpdateStock={(val) =>
                          handleQuickUpdate(product, "stock", val)
                        }
                        onUpdateThreshold={(val) =>
                          handleQuickUpdate(product, "alertThreshold", val)
                        }
                      />
                    </div>
                    <div className="grid grid-cols-4 gap-2 mt-auto">
                      <button
                        ...
                        className="py-2 bg-green-500/10 hover:bg-green-500 text-green-500 hover:text-white rounded-lg transition-colors flex items-center justify-center border border-green-500/30"
                      ...
                      <button
                        ...
                        className="py-2 bg-blue-500/10 hover:bg-blue-500 text-blue-500 hover:text-white rounded-lg transition-colors flex items-center justify-center border border-blue-500/30"
                        title="Historique"
                      >
                        <History size={16} />
                      </button>
                      <button
                        ...
                        className="py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg transition-colors flex items-center justify-center border border-slate-700"
                        title="Modifier"
                      >
                        <Edit2 size={16} />
                      </button>
                      <button
                        ...
                        className="py-2 bg-slate-800 hover:bg-red-600 text-slate-400 hover:text-white rounded-lg transition-colors flex items-center justify-center border border-slate-700"
                        title="Supprimer"
                      >
                        <Trash2 size={16} />
                      </button>
            ...
            <div className="p-6 overflow-y-auto">
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 mb-6">
                ...
                <StockProgressBar
                  ...
                  threshold={restockProduct.alertThreshold}
                />
              </div>
              <form onSubmit={handleRestockInitiate} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white font-bold text-xl focus:border-green-500 outline-none"
                    placeholder="0"
                    autoFocus
                  />
                </div>
                ...
                <button
                  type="submit"
                  className="w-full py-4 bg-green-600 hover:bg-green-500 text-white font-bold rounded-xl flex items-center justify-center gap-2"
                >
                  Valider l'entrée
                </button>
              </form>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/95 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div className="bg-slate-900 border border-green-500/30 w-full max-w-sm rounded-2xl shadow-2xl p-6 text-center animate-in zoom-in duration-300">
            ...
            <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 mb-6 text-left">
              ...
              <div className="flex justify-between items-center pt-2 border-t border-slate-800">
                ...
                <span className="text-white font-bold">
                  {restockProduct.stock + parseInt(restockQty)}
                </span>
              </div>
              {restockNote && (
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={handleRestockFinalConfirm}
                className="flex-1 py-3 rounded-xl bg-green-600 text-white font-bold hover:bg-green-500 transition-colors shadow-lg shadow-green-500/20"
              >
                Confirmer
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200 overflow-y-auto no-scrollbar">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-2xl rounded-2xl shadow-2xl p-6 relative my-8">
            ...
            <h2 className="text-lg md:text-xl font-bold text-white mb-6">
              {editingProduct ? "Détails du Produit" : "Nouveau Produit"}
            </h2>
            <form onSubmit={handleSave} className="space-y-6">
              <div className="space-y-6">
                ...
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl space-y-5">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2 text-bar-accent">
                      <Package size={16} />
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-slate-900 border border-slate-800 rounded-lg p-3 text-white text-lg focus:border-bar-accent outline-none font-bold shadow-inner"
                      />
                    </div>
                    <div>
                      <label className="block text-orange-400 text-[10px] font-bold mb-1.5 uppercase tracking-wide flex items-center gap-1">
                        Seuil d'Alerte <Info size={10} />
                      </label>
                      <input
                        type="number"
                        value={alertThreshold}
                        onChange={(e) => setAlertThreshold(e.target.value)}
                        className="w-full bg-slate-900 border border-orange-900/40 rounded-lg p-3 text-orange-400 text-lg focus:border-orange-500 outline-none font-bold shadow-inner"
                      />
                    </div>
                  </div>
                  <div className="pt-2">
                    <StockProgressBar
                      ...
                      threshold={parseInt(alertThreshold) || 1}
                    />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white font-bold focus:border-bar-accent outline-none"
                    />
                  </div>
                </div>
              </div>
              <div className="pt-6 border-t border-slate-800 flex flex-col sm:flex-row gap-3">
                ...
                <button
                  type="submit"
                  className="flex-1 py-3.5 rounded-xl bg-bar-accent text-white font-bold hover:bg-pink-600 shadow-lg shadow-bar-accent/20 transition-all"
                >
                  {editingProduct
      ...
      {isHistoryViewOpen && restockProduct && (
        <div className="fixed inset-0 z-[120] bg-slate-950 flex flex-col animate-in slide-in-from-bottom-4 duration-300">
          <div className="p-4 md:p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900">
            <h2 className="text-xl md:text-2xl font-black text-white flex items-center gap-3 italic">
              <History size={24} className="text-blue-500" /> JOURNAL DE STOCK
            </h2>
            <button
            ...
            <div className="max-w-4xl mx-auto">
              <div className="mb-8 flex justify-between items-end bg-slate-900 p-6 rounded-2xl border border-slate-800">
                ...
                <div className="text-right">
                  ...
                  <span
                    className={`text-3xl font-black ${restockProduct.stock <= restockProduct.alertThreshold ? "text-orange-500" : "text-green-500"}`}
                  >
              ...
              <div className="space-y-4">
                {stockHistory.length === 0 ? (
                  <div className="text-center py-16 text-slate-500 italic border-2 border-dashed border-slate-800 rounded-2xl font-medium">
                  ...
                  stockHistory.map((entry) => (
                    <div
                      key={entry.id}
                      className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex items-start gap-5 hover:border-blue-500/50 transition-colors shadow-lg shadow-black/20"
                    >
      ...
      {isGlobalHistoryOpen && (
        <div className="fixed inset-0 z-[120] bg-slate-950 flex flex-col animate-in slide-in-from-bottom-4 duration-300">
          <div className="p-4 md:p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900">
            <div>
              <h2 className="text-xl md:text-2xl font-black text-white flex items-center gap-3 italic">
                <ClipboardList size={24} className="text-blue-500" /> JOURNAL DE
                STOCK GLOBAL
              </h2>
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-8">
            <div className="max-w-5xl mx-auto">
              <div className="space-y-4">
                {globalStockHistory.length === 0 ? (
                  <div className="text-center py-16 text-slate-500 italic border-2 border-dashed border-slate-800 rounded-2xl font-medium">
                    Aucune entrée enregistrée dans le journal.
                  </div>
                ) : (
                  globalStockHistory.map((entry) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center mb-2">
                          <div>
                            <span className="font-black text-white text-lg mr-2">
                              +{entry.quantity} unités
                            </span>
                            <span className="text-sm text-slate-300 font-bold bg-slate-800 px-2 py-0.5 rounded">
