      ...
      >
        ...
        <div className="p-6 border-b border-slate-800 bg-slate-950/50">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-3">
              ...
              <span className="bg-bar-accent text-white text-[10px] font-black px-2 py-0.5 rounded-full">
                {totalItems}
              </span>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar">
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div
                key={item.id}
                className="flex justify-between items-center bg-slate-950/30 p-4 rounded-2xl border border-slate-800/50 group hover:border-bar-accent/50 transition-all"
              >
                <div className="flex items-center gap-4 flex-1">
                  <div className="flex items-center gap-2 bg-slate-900 rounded-xl p-1.5 border border-slate-800 shadow-inner">
                    ...
                    <span className="w-8 text-center text-white text-sm font-black">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() =>
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-bar-accent text-xs font-black">
        ...
        <div className="p-6 bg-slate-950 border-t border-slate-800">
          <div className="flex items-center justify-between mb-4">
            ...
            >
              <div
                className={`w-4 h-4 rounded-full bg-white absolute top-1 transition-transform ${isUrgent ? "left-7" : "left-1"}`}
              ></div>
            </button>
          </div>
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex-1 overflow-y-auto p-8 no-scrollbar">
              <div className="space-y-10">
                {Object.entries(tablesByZone).map(([zone, zoneTables]) => (
                  <div key={zone}>
                    <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.3em] mb-4 ml-1">
                      {zone}
                    </h3>
