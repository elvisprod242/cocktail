...
export const TablesPage: React.FC<TablesPageProps> = ({
  ...
  const [reservationModalTable, setReservationModalTable] =
    useState<TableDef | null>(null);
  ...
  const getTableCurrentOrder = (tableName: string) => {
    return orders.find(
      (o) => o.tableName === tableName && o.status !== OrderStatus.PAID,
    );
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6">
        <div>
          ...
          <p className="text-slate-400 mt-1 font-medium">
            Gérez vos espaces : Salle, Terrasse, Bar et VIP
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-6">
        {currentTables.length > 0 ? (
                  ...
                  {isOccupied && activeOrder ? (
                    <div className="animate-in fade-in duration-500">
                      ...
                      <p className="text-[10px] text-slate-500 font-bold flex items-center gap-1 mt-1 uppercase">
                        ...
                        Occupée{" "}
                        {Math.floor(
                          (Date.now() - activeOrder.timestamp) / 60000,
                        )}
                        m
                      </p>
                    </div>
                ...
                <div className="flex gap-2 mt-4">
                  {isOccupied ? (
                    ...
                    >
                      ...
                      {isReserved ? "Libérer" : "Réserver"}
                    </button>
                  )}
                  <button
                    onClick={() =>
                      navigate("/pos", { state: { tableName: table.name } })
                    }
                    className={`w-10 h-10 rounded-xl transition-all flex items-center justify-center border ${
                      isOccupied
                        ? "bg-slate-800 border-slate-700 text-white"
                        : "bg-slate-800 hover:bg-bar-accent border-slate-700 text-slate-400 hover:text-white"
                    }`}
          ...
          <div className="col-span-full py-20 text-center flex flex-col items-center gap-4 text-slate-600">
            ...
            <button
              onClick={() => setIsModalOpen(true)}
              className="text-bar-accent text-[10px] font-black uppercase hover:underline"
            >
          ...
          <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-3xl shadow-2xl p-8 animate-in zoom-in duration-300">
            ...
            <div className="mb-6 p-4 bg-slate-950 rounded-2xl border border-slate-800">
              ...
              <p className="text-xl font-black text-white">
                {reservationModalTable.name} ({reservationModalTable.zone})
              </p>
            </div>
            <div className="space-y-4 mb-8">
              <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest">
                Note (Nom, Heure, Couverts)
              </label>
              <input
                type="text"
                value={resNote}
            ...
            <div className="flex gap-4">
              <button
                onClick={() => setReservationModalTable(null)}
                className="flex-1 py-4 text-slate-400 font-bold uppercase tracking-widest text-xs"
              >
                Annuler
              </button>
              <button
                onClick={submitReservation}
                className="flex-1 py-4 bg-amber-500 hover:bg-amber-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg shadow-amber-900/20 transition-all"
              >
                Confirmer
              </button>
            </div>
            ...
            <form onSubmit={handleAddTable} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white focus:ring-2 focus:ring-bar-accent outline-none font-bold"
                  autoFocus
                  placeholder="Ex: T5, VIP-2..."
                />
              </div>
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button
                  type="submit"
                  className="flex-1 py-4 bg-bar-accent hover:bg-pink-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs transition-all shadow-lg shadow-bar-accent/20"
                >
                  Créer
                </button>
              </div>
