...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const bestDay = useMemo(() => {
    ...
    filteredOrders.forEach((o) => {
      const day = new Date(o.timestamp).toLocaleDateString("fr-FR", {
        weekday: "long",
      });
  ...
  const StatCard = ({
    ...
    <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-lg relative overflow-hidden group">
      ...
      <div className="relative z-10">
        ...
        {sub && (
          <p className={`text-xs ${subColor} flex items-center gap-1`}>{sub}</p>
        )}
      </div>
    </div>
  );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Vision globale de la performance de votre établissement
          </p>
        </div>
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-4">
            {topProducts.length === 0 ? (
              ...
              topProducts.map((prod, index) => (
                <div
                  key={prod.name}
                  className="flex items-center p-3 bg-slate-800/50 rounded-xl border border-slate-800"
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm mr-3 ${index === 0 ? "bg-yellow-500 text-black" : index === 1 ? "bg-slate-400 text-black" : index === 2 ? "bg-orange-700 text-white" : "bg-slate-700 text-slate-400"}`}
                  >
                    {index + 1}
                  </div>
              ...
              >
                ...
                <Bar dataKey="value" fill="#8884d8" radius={[4, 4, 0, 0]}>
                  {hourlyData.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={entry.value > 0 ? "#e94560" : "#1e293b"}
                    />
                  ))}
                </Bar>
              </BarChart>
