      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 md:mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Consultez et réimprimez vos tickets
          </p>
        </div>
                    ...
                    <td className="p-4 max-w-xs">
                      <div className="flex flex-wrap gap-1">
                        {order.items.slice(0, 3).map((item, i) => (
                          <span
                            key={i}
                            className="bg-slate-800 px-2 py-0.5 rounded text-xs text-slate-300 border border-slate-700"
                          >
      ...
      <div className="md:hidden space-y-4 pb-24">
        {filteredOrders.length === 0 ? (
          ...
          filteredOrders.map((order) => (
            <div
              key={order.id}
              className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-md active:scale-[0.99] transition-transform"
            >
              <div className="flex justify-between items-start mb-3 pb-3 border-b border-slate-800">
                ...
                <div className="flex flex-col items-end gap-1">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold border ${getStatusColor(order.status)}`}
                  >
                    {order.status}
                  </span>
                </div>
              ...
              <div className="mb-4">
                <div className="text-xs text-slate-500 mb-1 uppercase font-bold tracking-wider">
                  Articles
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {order.items.slice(0, 5).map((item, i) => (
                    <span
                      key={i}
                      className="bg-slate-950 px-2 py-1 rounded text-xs text-slate-300 border border-slate-800"
                    >
                      <span className="text-bar-accent font-bold mr-1">
                        {item.quantity}
                      </span>
                      {item.name}
                    </span>
