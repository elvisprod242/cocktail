...
export const Kitchen: React.FC<KitchenProps> = ({
  ...
  const [successPaymentOrder, setSuccessPaymentOrder] = useState<Order | null>(
    null,
  );
  ...
  const [notifications, setNotifications] = useState<
    { id: string; message: string; isUrgent: boolean }[]
  >([]);
  ...
  useEffect(() => {
    const newOrders = orders.filter(
      (o) => !knownOrderIds.has(o.id) && o.status === OrderStatus.PENDING,
    );
  ...
  const PaymentModal = () => {
    ...
    const changeDue =
      payingOrder && cashGiven ? parseFloat(cashGiven) - payingOrder.total : 0;

    // Find client for Tab calculation
    const currentClient = payingOrder
      ? clients.find((c) => c.id === payingOrder.clientId)
      : null;
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-in fade-in duration-200">
        <div
          className={`bg-slate-900 border border-slate-800 w-full rounded-2xl shadow-2xl overflow-hidden flex flex-col transition-all max-w-5xl max-h-[95vh]`}
        >
          <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900 shrink-0">
            <h2 className="text-xl font-bold text-white flex items-center gap-2">
              {step === "payment" ? (
                <Wallet className="text-bar-accent" />
              ) : (
                <CheckCircle className="text-green-500" />
              )}
              {step === "payment"
                ? `Encaissement Table ${payingOrder?.tableNumber}`
                : "Paiement Réussi"}
            </h2>
            <button
              onClick={handleClose}
                ...
                <div className="flex-1 flex flex-col overflow-y-auto pr-1">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-6 shrink-0">
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-3 rounded-lg border transition-all ${method === PaymentMethod.CASH ? "bg-green-600 border-green-500 text-white" : "bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700"}`}
                    >
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-3 rounded-lg border transition-all ${method === PaymentMethod.MOBILE_MONEY ? "bg-orange-600 border-orange-500 text-white" : "bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700"}`}
                    >
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-3 rounded-lg border transition-all ${method === PaymentMethod.CARD ? "bg-blue-600 border-blue-500 text-white" : "bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700"}`}
                    >
                      <CreditCard size={24} className="mb-1" />
                      <span className="text-xs font-bold">Carte</span>
                    </button>
                    <button
                      onClick={() => setMethod(PaymentMethod.TAB)}
                      disabled={!payingOrder.clientId}
                      className={`flex flex-col items-center justify-center p-3 rounded-lg border transition-all ${method === PaymentMethod.TAB ? "bg-purple-600 border-purple-500 text-white" : "bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700"} ${!payingOrder.clientId ? "opacity-50 cursor-not-allowed" : ""}`}
                    >
                  ...
                  <div className="bg-slate-950 rounded-xl p-4 border border-slate-800 mb-6 min-h-[120px] shrink-0">
                    {method === PaymentMethod.CASH && (
                      <div className="space-y-4">
                        ...
                        <div className="flex justify-between items-center pt-2 border-t border-slate-800">
                          ...
                          <span
                            className={`text-2xl font-bold ${changeDue < 0 ? "text-red-500" : "text-green-500"}`}
                          >
                    ...
                    {method === PaymentMethod.MOBILE_MONEY && (
                      <div className="text-center py-4">
                        ...
                        <p className="text-sm text-slate-400 mt-1">
                          Vérifiez le terminal ou le téléphone.
                        </p>
                      </div>
                    )}
                        ...
                        {currentClient ? (
                          <div className="w-full">
                            ...
                            <div className="bg-slate-900 rounded-lg p-4 border border-slate-800 max-w-md mx-auto">
                              ...
                              <div className="flex justify-between items-center text-base pt-1">
                                <span className="text-white font-bold">
                                  Nouveau Total Ardoise:
                                </span>
                                <span className="text-red-500 font-extrabold text-xl">
                                  {(-(
                                    currentClient.balance - payingOrder.total
                                  )).toFixed(2)}
                                  {currency}
                                </span>
                              </div>
                            ...
                            <p className="text-xs text-slate-500 mt-3">
                              Le montant sera ajouté à la dette du client.
                            </p>
                          </div>
                        ) : (
                    ...
                    {method === PaymentMethod.CARD && (
                      <div className="text-center py-4">
                        ...
                        />
                        <p className="text-white font-medium">
                          Utilisez le TPE pour encaisser.
                        </p>
                      </div>
                    )}
                  </div>
              ...
              <div className="flex flex-col items-center justify-center h-full py-8 space-y-6 animate-in slide-in-from-right duration-300">
                ...
                <div className="text-center">
                  ...
                  <p className="text-slate-400 text-lg">
                    Le ticket est prêt à l'impression.
                  </p>
                </div>
                ...
                <div className="fixed top-0 left-0 -left-[9999px] opacity-0 pointer-events-none">
                  {successPaymentOrder && (
                    <Receipt order={successPaymentOrder} currency={currency} />
                  )}
                </div>
              </div>
            )}
    ...
    <div className="p-4 md:p-8 h-full overflow-y-auto bg-slate-950">
          ...
          activeOrders.map((order) => {
            const isUrgent =
              ...
              (order.status === OrderStatus.PENDING &&
                Date.now() - order.timestamp > 10 * 60 * 1000);

            return (
              <div
                key={order.id}
                className={`bg-slate-900 border rounded-xl overflow-hidden flex flex-col shadow-lg animate-in fade-in zoom-in-95 duration-200 ${isUrgent ? "border-red-500 shadow-red-500/20" : "border-slate-800"}`}
              >
                <div
                  className={`p-4 border-b flex justify-between items-center ${isUrgent ? "bg-red-500/10 border-red-500/30" : "bg-slate-800/50 border-slate-800"}`}
                >
                  <div className="flex flex-col">
                    <span
                      className={`text-sm ${isUrgent ? "text-red-400" : "text-slate-400"}`}
                    >
                      Table
                    </span>
                  ...
                  <div className="flex flex-col items-end">
                    <div
                      className={`flex items-center gap-1 text-xs mb-1 ${isUrgent ? "text-red-400 font-bold" : "text-slate-400"}`}
                    >
                      <Clock size={12} />
                    ...
                    {order.items.map((item, idx) => (
                      ...
                      >
                        <div className="flex gap-3">
                          <span
                            className={`${isUrgent ? "bg-red-500" : "bg-slate-700"} text-white w-6 h-6 flex items-center justify-center rounded text-sm font-bold flex-shrink-0`}
                          >
                            {item.quantity}
                          </span>
                ...
                <div
                  className={`p-4 border-t ${isUrgent ? "bg-red-500/5 border-red-500/20" : "bg-slate-800/30 border-slate-800"}`}
                >
                  {order.status === OrderStatus.PENDING && (
                    <button
                      onClick={() =>
      ...
      <div className="fixed bottom-20 md:bottom-6 right-4 md:right-6 z-50 flex flex-col gap-2 pointer-events-none">
        {notifications.map((notif) => (
          <div
            key={notif.id}
            className={`flex items-center gap-3 px-4 py-3 rounded-xl shadow-2xl animate-in slide-in-from-right-full fade-in duration-300 pointer-events-auto ${notif.isUrgent ? "bg-red-500 text-white" : "bg-slate-800 border border-slate-700 text-white"}`}
          >
            {notif.isUrgent ? (
              <AlertTriangle size={20} />
            ) : (
              <Bell size={20} className="text-bar-accent" />
            )}
            <span className="font-bold text-sm">{notif.message}</span>
            {notif.isUrgent && (
              <span className="text-[10px] bg-white text-red-500 px-2 py-0.5 rounded-full font-black uppercase tracking-widest ml-2">
                Urgent
              </span>
            )}
          </div>
