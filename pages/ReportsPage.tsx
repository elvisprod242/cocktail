...
export const ReportsPage: React.FC<ReportsPageProps> = ({
  ...
  const reportData = useMemo(() => {
    const categories: Record<
      string,
      { total: number; qty: number; cost: number }
    > = {};
    const items: Record<
      string,
      { total: number; qty: number; category: string; cost: number }
    > = {};
    ...
    filteredOrders.forEach((order) => {
      if (order.paymentMethod) {
        payments[order.paymentMethod] =
          (payments[order.paymentMethod] || 0) + order.total;
      }
      ...
      order.items.forEach((item) => {
        ...
        if (!categories[catName])
          categories[catName] = { total: 0, qty: 0, cost: 0 };
        ...
        // Items stats
        if (!items[item.name])
          items[item.name] = { total: 0, qty: 0, category: catName, cost: 0 };
    ...
    return {
      ...
      items: Object.entries(items).sort(
        (a, b) => b[1].total - b[1].cost - (a[1].total - a[1].cost),
      ), // Sort by Profit
  ...
  const totalCost = reportData.items.reduce(
    (sum, [_, data]) => sum + data.cost,
    0,
  );
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `rapport_rentabilite_${period.toLowerCase()}.csv`,
    );
  ...
  const SummaryCard = ({ title, value, subValue, icon: Icon, color }: any) => (
    <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-lg relative overflow-hidden group">
      <div
        className={`absolute -right-2 -top-2 p-6 opacity-10 ${color} group-hover:scale-110 transition-transform duration-500`}
      >
        <Icon size={64} />
      </div>
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-8 gap-6">
        <div>
          ...
          <p className="text-slate-400 mt-1 font-medium">
            Suivi précis des marges basées sur les coûts d'achat historiques
          </p>
        </div>
            ...
            <table className="w-full text-left text-sm text-slate-400">
              ...
              <tbody className="divide-y divide-slate-800">
                {reportData.categories.map(([name, data]) => {
                  const profit = data.total - data.cost;
                  const margin =
                    data.total > 0 ? (profit / data.total) * 100 : 0;
                  return (
                    <tr
                      key={name}
                    ...
                    >
                      ...
                      <td className="p-5 text-right text-white font-black text-xs">
                        {data.total.toFixed(2)}
                        {currency}
                      </td>
                      ...
                      <td className="p-5 text-right text-green-500 font-bold text-xs">
                        +{profit.toFixed(2)}
                        {currency}
                      </td>
                      <td className="p-5 text-right">
                        <div className="flex flex-col items-end gap-1.5">
                          <span
                            className={`text-[10px] font-black px-2 py-0.5 rounded ${margin > 70 ? "bg-green-500/10 text-green-500" : margin > 40 ? "bg-blue-500/10 text-blue-500" : "bg-red-500/10 text-red-500"}`}
                          >
                            {margin.toFixed(1)}%
                          </span>
                        </div>
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span
                      className={`text-sm font-black ${mult >= 3 ? "text-green-500" : "text-amber-500"}`}
                    >
                      x{mult.toFixed(2)}
                    </span>
                  </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl mb-24 md:mb-0">
        <div className="p-6 border-b border-slate-800 bg-slate-800/20 flex justify-between items-center">
          ...
          <span className="text-[10px] text-slate-500 font-bold italic">
            Basé sur les coûts réels à la vente
          </span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-400">
            ...
            <tbody className="divide-y divide-slate-800">
              {reportData.items.map(([name, data]) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-5 text-right text-white font-mono text-xs">
                      {data.total.toFixed(2)}
                      {currency}
                    </td>
                    <td className="p-5 text-right">
                    ...
                    <td className="p-5 text-right">
                      <span className="text-blue-400/80 font-medium text-[10px]">
                        {itemMargin.toFixed(1)}%
                      </span>
                    </td>
                    <td className="p-5 text-right text-slate-600 font-mono text-xs">
                      {data.cost.toFixed(2)}
                      {currency}
                    </td>
                    <td className="p-5 text-right">
                    ...
                    <td className="p-5 text-right">
                      <span
                        className={`text-[10px] font-black ${multiplier >= 3 ? "text-blue-400" : "text-slate-500"}`}
                      >
                    ...
                    <td className="p-5 text-right">
                      <span
                        className={`text-[10px] font-black px-2 py-1 rounded-lg ${itemMargin > 75 ? "bg-green-500/20 text-green-500 border border-green-500/30" : "bg-slate-800 text-slate-500"}`}
                      >
              ...
              {reportData.items.length === 0 && (
                <tr>
                  <td
                    colSpan={9}
                    className="p-20 text-center italic text-slate-600 uppercase font-black tracking-widest opacity-20"
                  >
